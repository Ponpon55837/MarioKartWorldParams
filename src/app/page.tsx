    ...
    return (
      <div className="flex justify-center items-center min-h-screen">
        <div className="bg-red-100 border border-red-400 text-red-700 px-6 py-4 rounded-lg text-center max-w-md">
          ...
          <button onClick={() => window.location.reload()} className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded transition-colors">
            {t('loading.retry')}
          </button>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-md p-4 border border-gray-200">
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm text-gray-500">{t('stats.total', { charactersCount: characters.length, vehiclesCount: vehicles.length })}</div>
          </div>
        </div>
        ...
        {currentPage === 'combinations' && (
          <section>
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-bold text-gray-800 text-center flex-1">⭐ {t('stats.combinationCount', { count: combinations.length })}</h2>
              {combinations.length > 0 && (
                <button onClick={handleClearCombinations} className="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors text-sm">
                  🗑️ {t('stats.clearAll')}
                </button>
              )}
            </div>
            ...
            {/* 組合選擇器 */}
            <CombinationSelector characters={characters} vehicles={vehicles} onAddCombination={addCombination} />
            ...
            {combinations.length === 0 ? (
              <div className="text-center py-8 bg-gray-50 rounded-lg mt-4">
                ...
                <div className="text-sm text-gray-500 bg-white p-3 rounded-lg border border-gray-200 inline-block">💡 {t('emptyCombination.tip')}</div>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
                {combinations.map((combination) => (
                  <CombinationCard key={combination.id} character={combination.character} vehicle={combination.vehicle} onRemove={() => handleRemoveCombination(combination.id)} />
                ))}
              </div>
            )}
          </section>
        ...
        {currentPage === 'characters' && (
          <section>
            <h2 className="text-2xl font-bold text-gray-800 mb-4 text-center">🎮 {t('stats.characterCount', { count: sortedCharacters.length })}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {sortedCharacters.map((character) => (
                <CharacterCard key={character.name} character={character} maxStats={maxStats} speedFilter={speedFilter} handlingFilter={handlingFilter} />
              ))}
            </div>
          </section>
        )}
        ...
        {currentPage === 'vehicles' && (
          <section>
            <h2 className="text-2xl font-bold text-gray-800 mb-4 text-center">🏎️ {t('stats.vehicleCount', { count: sortedVehicles.length })}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {sortedVehicles.map((vehicle) => (
                <VehicleCard key={vehicle.name} vehicle={vehicle} maxStats={maxStats} speedFilter={speedFilter} handlingFilter={handlingFilter} />
              ))}
            </div>
          </section>
        )}
        ...
        <div className="text-center mb-2">
          <a href="/admin" className="inline-flex items-center px-3 py-1 text-xs text-gray-500 hover:text-blue-600 transition-colors border border-gray-300 rounded-full hover:border-blue-300">
