...
```json
{
  "dependencies": {
    ...
    "next": "16.2.3", // ✅ 已有
    "react": "^18.3.1", // ✅ 已有
    "@tanstack/react-query": "^5.0.0", // ⭐ 需新增（資料獲取）
    "react-hook-form": "^7.51.0", // ⭐ 需新增（表單管理）
    "zod": "^3.22.0", // ⭐ 需新增（驗證 Schema）
    "@hookform/resolvers": "^3.3.0" // ⭐ 需新增（整合橋接）
  }
}
```
...
export const profileSchema = z.object({
  ...
  avatar: z
    .instanceof(File)
    .refine((file) => file.size <= 5 * 1024 * 1024, '檔案大小不能超過 5MB')
    .refine((file) => ['image/jpeg', 'image/png', 'image/webp'].includes(file.type), '只支援 JPG、PNG、WebP 格式')
    .optional(),
  bio: z.string().max(500, '個人簡介最多 500 字元').optional()
});
...
export default function LoginForm() {
  ...
  const {
    ...
    resolver: zodResolver(loginSchema), // 整合 Zod
    mode: 'onBlur' // 失焦時驗證
  })
            ...
            {password && (
              <div className="mt-2">
                <div className="flex gap-1">
                  ...
                  <div className={`h-1 flex-1 rounded ${/[A-Z]/.test(password) ? 'bg-green-500' : 'bg-gray-200'}`} />
                  <div className={`h-1 flex-1 rounded ${/[0-9]/.test(password) ? 'bg-green-500' : 'bg-gray-200'}`} />
                </div>
                <p className="text-xs text-gray-500 mt-1">
                  密碼強度：至少 8 字元、包含大寫字母和數字
                </p>
              </div>
            )}
          </div>
...
export default function SurveyPage() {
  ...
  const { register, handleSubmit, formState: { errors }, watch } = useForm<SurveyFormData>({
...
export const teamSchema = z.object({
  teamName: z.string().min(2, '隊伍名稱至少 2 個字元'),
  members: z
    .array(memberSchema)
    .min(2, '隊伍至少需要 2 位成員')
    .max(10, '隊伍最多 10 位成員')
...
function TeamForm() {
  const { control, register, handleSubmit } = useForm({
    ...
    defaultValues: {
      teamName: '',
      members: [{ name: '', role: 'leader', email: '' }]
    }
  })
  ...
  return (
    <form>
      ...
      <button
        type="button"
        onClick={() => append({ name: '', role: 'member', email: '' })}
      >
        新增成員
      </button>
    </form>
...
// 註冊表單草稿
export const registerDraftAtom = atomWithStorage<Partial<RegisterFormData> | null>(
...
function OrganizationSelector() {
  ...
  const { data: personDetail } = usePersonDetail()

  return (
    <div className="space-y-4">
      {/* 部門下拉選單 */}
      <select onChange={(e) => setDepartment(e.target.value || null)}>
      ...
      {selectedDepartmentId && (
        <select onChange={(e) => setSelectedGroupId(e.target.value || null)}>
...
function CreateCharacterForm() {
  ...
  const handleSubmit = (data: NewCharacter) => {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      >
        {createMutation.isLoading ? '新增中...' : '新增角色'}
      </button>
    </form>
  )
}
...
import { useAtom } from 'jotai'
import { sortByAtom } from '@/store/dataAtoms'
...
const form = useForm({
  mode: 'onBlur',
  reValidateMode: 'onChange' // 第一次驗證後改為即時
});
```
...
function CharactersList({ page }: { page: number }) {
  ...
  const { data } = useQuery({
    ...
    queryFn: () => authFetch(`/api/characters?page=${page}`)
  })
  ...
  useEffect(() => {
    queryClient.prefetchQuery({
      ...
      queryFn: () => authFetch(`/api/characters?page=${page + 1}`)
    })
  }, [page, queryClient])
...
- 瀏覽器 API 互動（resize, scroll）
- 第三方庫整合
- 本地副作用（document.title）
...
async function refreshAccessToken() {
  if (isRefreshing && refreshPromise) {
    return refreshPromise; // ← 等待第一個請求的結果
  }
...
4. **自動化優先**
   - 讓 Tanstack Query 處理快取、重試、競態
   - 移除手動的 useEffect 資料獲取
...
1. 安裝 Tanstack Query: `pnpm add @tanstack/react-query`
2. 實現 authFetch 工具
3. 建立 AuthProvider
4. 整合到專案 Layout
5. 逐步將現有的資料獲取邏輯遷移到 useQuery
